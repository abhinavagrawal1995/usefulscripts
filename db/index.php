<?php
/** system_admin - Compact database management for System Admin MIT
Edited by Abhinav Agrawal
...
system_admin(){global$b;return$b;}function
...
set_password($Nh,$N,$V,$G){$_SESSION["pwds"][$Nh][$N][$V]=($_COOKIE["system_admin_key"]&&is_string($G)?array(encrypt_string($G,$_COOKIE["system_admin_key"])):$G);}function
get_password(){$J=get_session("pwds");if(is_array($J))$J=($_COOKIE["system_admin_key"]?decrypt_string($J[0],$_COOKIE["system_admin_key"]):false);return$J;}function
...
password_file($yb){$Dc=get_temp_dir()."/system_admin.key";$J=@file_get_contents($Dc);if($J||!$yb)return$J;$Nc=@fopen($Dc,"w");if($Nc){$J=rand_string();fwrite($Nc,$J);fclose($Nc);}return$J;}function
rand_string(){return
md5(uniqid(mt_rand(),true));}function
select_value($X,$_,$n,$ah){global$b,$ba;if(is_array($X)){$J="";foreach($X
as$Bd=>$W)$J.="<tr>".($X!=array_values($X)?"<th>".h($Bd):"")."<td>".select_value($W,$_,$n,$ah);return"<table cellspacing='0'>$J</table>";}if(!$_)$_=$b->selectLink($X,$n);if($_===null){if(is_mail($X))$_="mailto:$X";if($Df=is_url($X))$_=(($Df=="http"&&$ba)||preg_match('~WebKit~i',$_SERVER["HTTP_USER_AGENT"])?$X:"$Df://www.system_admin.org/redirect/?url=".urlencode($X));}$J=$b->editVal($X,$n);if($J!==null){if($J==="")$J="&nbsp;";elseif($ah!=""&&is_shortable($n)&&is_utf8($J))$J=shorten_utf8($J,max(0,+$ah));else$J=h($J);}return$b->selectVal($J,$_,$n,$X);}function
...
';}global$b,$g,$Sb,$ac,$kc,$m,$Qc,$Vc,$ba,$pd,$w,$ca,$Jd,$He,$nf,$Eg,$Zc,$T,$oh,$vh,$Bh,$ia;if(!$_SERVER["REQUEST_URI"])$_SERVER["REQUEST_URI"]=$_SERVER["ORIG_PATH_INFO"];if(!strpos($_SERVER["REQUEST_URI"],'?')&&$_SERVER["QUERY_STRING"]!="")$_SERVER["REQUEST_URI"].="?$_SERVER[QUERY_STRING]";$ba=$_SERVER["HTTPS"]&&strcasecmp($_SERVER["HTTPS"],"off");@ini_set("session.use_trans_sid",false);session_cache_limiter("");if(!defined("SID")){session_name("system_admin_sid");$F=array(0,preg_replace('~\\?.*~','',$_SERVER["REQUEST_URI"]),"",$ba);if(version_compare(PHP_VERSION,'5.2.0')>=0)$F[]=true;call_user_func_array('session_set_cookie_params',$F);session_start();}remove_slashes(array(&$_GET,&$_POST,&$_COOKIE),$Ec);if(get_magic_quotes_runtime())set_magic_quotes_runtime(false);@set_time_limit(0);@ini_set("zend.ze1_compatibility_mode",false);@ini_set("precision",20);function
...
false;}$_->query('PRAGMA encoding = "UTF-8"');$_->query('CREATE TABLE system_admin (i)');$_->query('DROP TABLE system_admin');return
true;}function
...
as$x=>$n)$o[$x]="  ".implode($n);$o=array_merge($o,array_filter($Hc));if(!queries("CREATE TABLE ".table($Q!=""?"system_admin_$C":$C)." (\n".implode(",\n",$o)."\n)"))return
false;if($Q!=""){if($Ye&&!queries("INSERT INTO ".table("system_admin_$C")." (".implode(", ",$Ye).") SELECT ".implode(", ",array_map('idf_escape',array_keys($Ye)))." FROM ".table($Q)))return
...
false;queries("ALTER TABLE ".table("system_admin_$C")." RENAME TO ".table($C));if(!alter_indexes($C,$v))return
false;foreach($sh
...
Min_DB;$i=$b->credentials();if($g->connect($i[0],$i[1],$i[2])){if($g->server_info>=9)$g->query("SET application_name = 'system_admin'");return$g;}return$g->error;}function
get_databases(){return
...
system_admin{var$operators;function
name(){return"<a href='http://sysad.techtatva.in/' target='_blank' id='h1'>system_admin</a>";}function
credentials(){return
...
echo"<p><input type='submit' value='".'Login'."'>\n",checkbox("auth[permanent]",1,$_COOKIE["system_admin_permanent"],'Permanent login')."\n";}function
login($Ud,$G){return
true;}function
tableName($Lg){return
h($Lg["Name"]);}function
...
<a href="http://sysad.techtatva.in/#download" target="_blank" id="version">',(version_compare($ia,$_COOKIE["system_admin_version"])<0?h($_COOKIE["system_admin_version"]):""),'</a>
</h1>
...
';if(file_exists("system_admin.css")){echo'<link rel="stylesheet" type="text/css" href="system_admin.css">
';}}echo'
<body class="ltr nojs" style="background-image: url(\'back.jpg\');" onkeydown="bodyKeydown(event);" onclick="bodyClick(event);"',(isset($_COOKIE["system_admin_version"])?"":" onload=\"verifyVersion('$ia');\""),'>
<script type="text/javascript">
...
long2str($W,true);}$g='';$Zc=$_SESSION["token"];if(!$Zc)$_SESSION["token"]=rand(1,1e6);$T=get_token();$nf=array();if($_COOKIE["system_admin_permanent"]){foreach(explode(" ",$_COOKIE["system_admin_permanent"])as$X){list($x)=explode(":",$X);$nf[$x]=$X;}}function
add_invalid_login(){global$b;$Dc=get_temp_dir()."/system_admin.invalid";$Nc=@fopen($Dc,"r+");if(!$Nc){$Nc=@fopen($Dc,"w");if(!$Nc)return;}flock($Nc,LOCK_EX);$ud=unserialize(stream_get_contents($Nc));$bh=time();if($ud){foreach($ud
as$vd=>$X){if($X[0]<$bh)unset($ud[$vd]);}}$td=&$ud[$b->bruteForceKey()];if(!$td)$td=array($bh+30*60,0);$td[1]++;$qg=serialize($ud);rewind($Nc);fwrite($Nc,$qg);ftruncate($Nc,strlen($qg));flock($Nc,LOCK_UN);fclose($Nc);}$Ia=$_POST["auth"];if($Ia){$ud=unserialize(@file_get_contents(get_temp_dir()."/system_admin.invalid"));$td=$ud[$b->bruteForceKey()];$xe=($td[1]>30?$td[0]-time():0);if($xe>0)auth_error(lang(array('Too many unsuccessful logins, try again in %d minute.','Too many unsuccessful logins, try again in %d minutes.'),ceil($xe/60)));session_regenerate_id();$l=$Ia["driver"];$N=$Ia["server"];$V=$Ia["username"];$G=(string)$Ia["password"];$k=$Ia["db"];set_password($l,$N,$V,$G);$_SESSION["db"][$l][$N][$V][$k]=true;if($Ia["permanent"]){$x=base64_encode($l)."-".base64_encode($N)."-".base64_encode($V)."-".base64_encode($k);$zf=$b->permanentLogin(true);$nf[$x]="$x:".base64_encode($zf?encrypt_string($G,$zf):"");cookie("system_admin_permanent",implode(" ",$nf));}if(count($_POST)==1||DRIVER!=$l||SERVER!=$N||$_GET["username"]!==$V||DB!=$k)redirect(auth_url($l,$N,$V,$k));}elseif($_POST["logout"]){if($Zc&&!verify_token()){page_header('Logout','Invalid CSRF token. Send the form again.');page_footer("db");exit;}else{foreach(array("pwds","db","dbs","queries")as$x)set_session($x,null);unset_permanent();redirect(substr(preg_replace('~\b(username|db|ns)=[^&]*&~','',ME),0,-1),'Logout successful.');}}elseif($nf&&!$_SESSION["pwds"]){session_regenerate_id();$zf=$b->permanentLogin();foreach($nf
...
as$x=>$X)echo"<tr valign='top'><td>".h($X[0])."<td>".h($X[1])."<th>".h($x)."<td><a href='".h(ME.'trigger='.urlencode($a).'&name='.urlencode($x))."'>".'Alter'."</a>\n";echo"</table>\n";}echo'<p class="links"><a href="'.h(ME).'trigger='.urlencode($a).'">'.'Add trigger'."</a>\n";}}elseif(isset($_GET["schema"])){page_header('Database schema',"",array(),h(DB.($_GET["ns"]?".$_GET[ns]":"")));$Og=array();$Pg=array();$C="system_admin_schema";$ea=($_GET["schema"]?$_GET["schema"]:$_COOKIE[($_COOKIE["$C-".DB]?"$C-".DB:$C)]);preg_match_all('~([^:]+):([-0-9.]+)x([-0-9.]+)(_|$)~',$ea,$Yd,PREG_SET_ORDER);foreach($Yd
...
';}elseif(isset($_GET["dump"])){$a=$_GET["dump"];if($_POST&&!$m){$wb="";foreach(array("output","format","db_style","routines","events","table_style","auto_increment","triggers","data_style")as$x)$wb.="&$x=".urlencode($_POST[$x]);cookie("system_admin_export",substr($wb,1));$S=array_flip((array)$_POST["tables"])+array_flip((array)$_POST["data"]);$wc=dump_headers((count($S)==1?key($S):DB),(DB==""||count($S)>1));$xd=preg_match('~sql~',$_POST["format"]);if($xd){echo"-- system_admin $ia ".$Sb[DRIVER]." dump\n\n";if($w=="sql"){echo"SET NAMES utf8;
SET time_zone = '+00:00';
...
';}elseif(isset($_GET["view"])){$a=$_GET["view"];$K=$_POST;if($_POST&&!$m){$C=trim($K["name"]);$Ea=" AS\n$K[select]";$A=ME."table=".urlencode($C);$ge='View has been altered.';if(!$_POST["drop"]&&$a==$C&&$w!="sqlite")query_redirect(($w=="mssql"?"ALTER":"CREATE OR REPLACE")." VIEW ".table($C).$Ea,$A,$ge);else{$Wg=$C."_system_admin_".uniqid();drop_create("DROP VIEW ".table($a),"CREATE VIEW ".table($C).$Ea,"DROP VIEW ".table($C),"CREATE VIEW ".table($Wg).$Ea,"DROP VIEW ".table($Wg),($_POST["drop"]?substr(ME,0,-1):$A),'View has been dropped.',$ge,'View has been created.',$a,$C);}}if(!$_POST&&$a!=""){$K=view($a);$K["name"]=$a;if(!$m)$m=$g->error;}page_header(($a!=""?'Alter view':'Create view'),$m,array("table"=>$a),h($a));echo'
<form action="" method="post">
...
';}elseif(isset($_GET["select"])){$a=$_GET["select"];$R=table_status1($a);$v=indexes($a);$o=fields($a);$Jc=column_foreign_keys($a);$Be="";if($R["Oid"]){$Be=($w=="sqlite"?"rowid":"oid");$v[]=array("type"=>"PRIMARY","columns"=>array($Be));}parse_str($_COOKIE["system_admin_import"],$wa);$Zf=array();$f=array();$ah=null;foreach($o
...
as$x=>$X){if(in_array(idf_escape($x),$M))unset($Ah[$x]);}if($_POST["export"]){cookie("system_admin_import","output=".urlencode($_POST["output"])."&format=".urlencode($_POST["format"]));dump_headers($a);$b->dumpTable($a,"");if(!is_array($_POST["check"])||$Ah===array())$H="SELECT $Oc$Uh$Uc";else{$xh=array();foreach($_POST["check"]as$X)$xh[]="(SELECT".limit($Oc,"\nWHERE ".($Z?implode(" AND ",$Z)." AND ":"").where_check($X,$o).$Uc,1).")";$H=implode(" UNION ALL ",$xh);}$b->dumpData($a,"table",$H);exit;}if(!$b->selectEmailProcess($Z,$Jc)){if($_POST["save"]||$_POST["delete"]){$I=true;$xa=0;$O=array();if(!$_POST["delete"]){foreach($f
